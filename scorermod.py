...
# this file is a modified version of the existing cribbage scoring library.
# The only change between the two is that this file does not account for the starter card.
# This change was necessary in order to use the scoring functions in more diverse ways, allowing us to score partial rounds easily.
...
def show_calc_score(hand, crib=False):
    scores = []

    card_nums = sorted(get_card_numbers(hand))
    hand_suits = get_card_suits(hand)

    scores.append(score_multiples(card_nums))
    scores.append(score_fifteens(hand))
    scores.append(score_runs(card_nums))
    scores.append(score_flushes(hand_suits, crib))
...
def score_flushes(hand_suits, crib):
    running_score = []
    running_msg = []

    multiples_list = count_multiples(hand_suits)
    if len(set(multiples_list)) == 1:
        if len(multiples_list) == 4 and not crib:
